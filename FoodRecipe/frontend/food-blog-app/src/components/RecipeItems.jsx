...
export default function RecipeItems() {
    ...
    const onDelete=async (id)=>{
      await axios.delete(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/recipe/${id}`)
          ...
          allRecipes?.map((item,index)=>{
            return(
              <div key={index} className='card' onDoubleClick={()=>navigate(`/recipe/${item._id}`)}> 
                  <img src={`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/images/${item.coverImage}`} width="120px" height="100px"></img>
